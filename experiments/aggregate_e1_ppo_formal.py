...
ALGORITHMS = {
    "Hourly Centralized Maskable PPO": "ppo_hourly",
    "High-level Centralized Maskable PPO": "ppo_high_level",
    "Event-Residual PPO": "ppo_event_residual",
}
...
def main() -> None:
    ...
    selected_algorithms = {
        algorithm: directory_name
        for algorithm, directory_name in ALGORITHMS.items()
        if (args.formal_root / directory_name).is_dir()
    }
    if not selected_algorithms:
        raise ValueError(
            f"No supported PPO result directory under {args.formal_root}."
        )
    episode_rows: list[dict[str, Any]] = []
    provenance: dict[str, Any] = {}
    for algorithm, directory_name in selected_algorithms.items():
