"""Project pipeline transport flows and distribute outflow to injection wells.

预测管道运输流量，并将输出流分配给注入井。
"""
...
def project_pipeline_outflow(
    ...
    annual_capacity_t = pipeline_annual_remaining_capacity_t(
        network,
        pipeline,
        state,
    )
...
def pipeline_annual_remaining_capacity_t(network, pipeline: Pipeline, state: PhysicalState) -> float:
    """Return remaining rolling-year pipeline capacity in tonnes.

    返回滚动年度内剩余的管道输送能力，单位为吨。
    """
...
def pipeline_minimum_stable_injection_t(network, pipeline_id: str, state: PhysicalState) -> float:
    """Return the smallest feasible non-zero injection quantity downstream.
