...
from .environment.event_driven import DecisionEvent, EventDrivenCCSEnv

Policy = Callable[[CCSEnv], dict[str, list]]
EventPolicy = Callable[[EventDrivenCCSEnv, DecisionEvent, list[float]], dict[str, list]]
...
def run_event_episode(
    ...
    """Evaluate an event policy with the same hourly KPI recorder as baselines."""
    transition = env.reset(seed=seed)
    recorder = _MetricsRecorder(env.env)
    for hourly in transition.hourly_transitions:
        recorder.record_step(hourly.reward, hourly.info)
    while transition.decision_event is not None:
        action = policy(env, transition.decision_event, transition.observation)
