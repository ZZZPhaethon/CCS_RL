...
from .entities.line_source import LineSourceParameters
...
def build_fixed_scenario_demo(identifier: str) -> tuple[PhysicalNetwork, PhysicalState]:
    """Load a fixed-network scenario from the project data directory."""
    scenario_id = resolve_fixed_scenario_id(identifier)
    hourly_profiles = {}
    if (
        scenario_id in {
        ...
        and NORTHERN_LIGHTS_PHASE1_CAPTURE_PROFILE_PATH.is_file()
    ):
        hourly_profiles = _load_hourly_capture_profiles(
