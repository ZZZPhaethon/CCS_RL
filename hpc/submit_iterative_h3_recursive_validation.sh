...
#SBATCH -o logs/iterative_h3_reval-%A_%a.out
#SBATCH -e logs/iterative_h3_reval-%A_%a.err
...
: "${MODEL_STAGE:?MODEL_STAGE must be set}"
...
CHECKPOINT="$OUT_ROOT/branches/$VARIANT/model_seed_${MODEL_SEED}/$MODEL_STAGE/iterative_action_q.pt"
OUT_DIR="$OUT_ROOT/validation/$VARIANT/model_seed_${MODEL_SEED}/$MODEL_STAGE"
