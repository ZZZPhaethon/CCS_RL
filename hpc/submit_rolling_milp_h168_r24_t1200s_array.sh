#!/usr/bin/env bash
#SBATCH --job-name=rolling_h168_t1200
...
#SBATCH --array=0-29%1
#SBATCH -o logs/rolling_h168_t1200-%A_%a.out
#SBATCH -e logs/rolling_h168_t1200-%A_%a.err
...
if (( ${SLURM_CPUS_PER_TASK:-0} != 4 )); then
  echo "H168 Rolling MILP requires exactly four allocated CPUs." >&2
  exit 2
fi
...
if [[ -e "$OUT_DIR" ]]; then
  echo "Refusing to overwrite H168-T1200 result directory: $OUT_DIR" >&2
  exit 3
fi
...
echo "planning_horizon_hours=168"
echo "replan_interval_hours=24"
echo "time_limit_seconds_per_replan=1200"
...
python -u experiments/smoke_test_paper_controllers.py \
  ...
  --rolling-time-limit-seconds 1200 \
  --solver-threads 4 \
  --purpose "$PURPOSE"
...
assert row["solver_time_limit_seconds_per_replan"] == 1200.0
...
metadata = {
    ...
    "solver_threads": 4,
    "solver_time_limit_seconds_per_replan": 1200,
...
(out_dir / "h168_r24_t1200s_task_metadata.json").write_text(
    json.dumps(metadata, indent=2) + "\n",
    encoding="utf-8",
)
print("h168_r24_t1200s_output_check=passed")
