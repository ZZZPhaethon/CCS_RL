...
RUN_ROOT="${RUN_ROOT:-output/iterative_q_validation_search/iterative_q_v2_continuous_teacher50}"
...
train_job=$(sbatch --parsable \
  ...
  --job-name="iterq_v2_linear50_train" \
  --export=ALL,PROJECT_DIR="$PROJECT_DIR",RUN_ROOT="$RUN_ROOT",DATA_ROOT="$DATA_ROOT",OUTPUT_STAGE="linear_050",DATA_STAGES="g0:g1:g2:g3",INITIAL_CHECKPOINT="$DATA_ROOT/p3/iterative_action_q.pt",ENCODER_LR=0.00003,HEAD_LR=0.0001,FOLLOW_ANCHOR=0.0,PREVIOUS_POLICY_ANCHOR=1.0,PREVIOUS_POLICY_RELEASE_MARGIN_EUR=50000,PREVIOUS_POLICY_ANCHOR_TEMPERATURE=0.5,PREVIOUS_POLICY_ANCHOR_WEIGHTING=linear,OBSERVATION_INPUT=shared_future_summary,MODEL_SEED=0 \
  hpc/submit_iterative_q_train.sh)
eval_job=$(sbatch --parsable \
  --dependency=afterok:"$train_job" \
  --job-name="iterq_v2_linear50_eval" \
  --export=ALL,PROJECT_DIR="$PROJECT_DIR",RUN_ROOT="$RUN_ROOT",FINAL_STAGE="linear_050",EVAL_NAME="iterative_q_v2_continuous_teacher50",SCENARIO_PROTOCOL=unified_window_v1,HARD_SCENARIO_PROBABILITY=0.5,FORECAST_CONTEXT_HOURS=168,POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES=12,EVAL_SEEDS="$EVAL_SEEDS",VALIDATION_ONLY=1 \
...
{
  printf 'name=Iterative Q v2 continuous teacher\n'
  printf 'data_root=%s\n' "$DATA_ROOT"
  printf 'initial_checkpoint=%s\n' \
    "$DATA_ROOT/p3/iterative_action_q.pt"
  printf 'data_stages=g0:g1:g2:g3\n'
  printf 'anchor_coefficient=1.0\n'
  printf 'anchor_weighting=linear\n'
  printf 'teacher_weight=max(0,1-exact_improvement_eur/50000)\n'
