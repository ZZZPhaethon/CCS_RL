...
for model_seed in $MODEL_SEEDS; do
  ...
  train_job=$(sbatch --parsable \
    ...
    hpc/submit_iterative_q_state_individual_retrain.sh)
  train_job="${train_job%%;*}"
  eval_job=$(sbatch --parsable \
    --dependency=afterok:"$train_job" \
    --export="$common_export" \
    hpc/submit_iterative_q_state_individual_eval.sh)
  eval_job="${eval_job%%;*}"
  {
...
{
  printf 'purpose=individual fixed-root retraining ablations for iterative Q state features\n'
  printf 'conditions=drop_in_transit_fill,drop_episode_progress\n'
  printf 'drop_in_transit_fill=hour_of_week,in_transit_fill\n'
  printf 'drop_episode_progress=hour_of_week,episode_progress\n'
  printf 'baseline=drop_hour_of_week from 20260730 fixed-root retraining\n'
  printf 'joint_ablation=drop_all_three from 20260730 fixed-root retraining\n'
  printf 'data_root=%s\n' "$DATA_ROOT"
