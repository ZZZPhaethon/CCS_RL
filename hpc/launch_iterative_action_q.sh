...
COLLECTION_REQUIRED_HEADS="${COLLECTION_REQUIRED_HEADS:-4}"
...
P1_RESIDUAL_MARGIN="${P1_RESIDUAL_MARGIN:-0.10}"
P1_ECONOMIC_MARGIN_EUR="${P1_ECONOMIC_MARGIN_EUR:-10000}"
ITER_RESIDUAL_MARGIN="${ITER_RESIDUAL_MARGIN:-0.15}"
ITER_ECONOMIC_MARGIN_EUR="${ITER_ECONOMIC_MARGIN_EUR:-15000}"
RESUME_FROM_P1="${RESUME_FROM_P1:-0}"
RESUME_FROM_G0="${RESUME_FROM_G0:-0}"
DRY_RUN="${DRY_RUN:-0}"
EVAL_SEEDS="${EVAL_SEEDS:-}"
VALIDATION_ONLY="${VALIDATION_ONLY:-0}"
MODEL_SEED="${MODEL_SEED:-0}"
EXCLUDE_STATE_FEATURES="${EXCLUDE_STATE_FEATURES:-}"
EVAL_EACH_STAGE="${EVAL_EACH_STAGE:-0}"
...
POLICY_WINDOW_COUNT=${#POLICY_WINDOW_VALUES[@]}
...
if ! [[ "$MAX_OVERRIDES" =~ ^[1-9][0-9]*$ ]] \
  || (( MAX_OVERRIDES > POLICY_WINDOW_COUNT )); then
  echo "MAX_OVERRIDES must be positive and no greater than the policy window count" >&2
  exit 2
fi
if ! [[ "$COLLECTION_REQUIRED_HEADS" =~ ^[1-5]$ ]]; then
...
printf 'scenario_protocol=%s hard_probability=%s context_hours=%s\n' \
  "$SCENARIO_PROTOCOL" "$HARD_SCENARIO_PROBABILITY" \
  "$FORECAST_CONTEXT_HOURS"
printf 'future_summary_windows_h=%s\n' "$FUTURE_SUMMARY_WINDOWS_H"
printf 'observation_input=%s\n' "$OBSERVATION_INPUT"
printf 'exclude_state_features=%s\n' "$EXCLUDE_STATE_FEATURES"
printf 'p1_margin=%s (%s EUR) iteration_margin=%s (%s EUR)\n' \
  "$P1_RESIDUAL_MARGIN" "$P1_ECONOMIC_MARGIN_EUR" \
...
if [[ "$RESUME_FROM_P1" == "1" && "$RESUME_FROM_G0" == "1" ]]; then
  echo "RESUME_FROM_P1 and RESUME_FROM_G0 are mutually exclusive" >&2
  exit 2
fi
...
if [[ "$RESUME_FROM_P1" == "1" ]]; then
  if [[ ! -s "$RUN_ROOT/p1/iterative_action_q.pt" \
     || ! -s "$RUN_ROOT/g0/train_merged.npz" \
     || ! -s "$RUN_ROOT/g0/validation_merged.npz" ]]; then
    echo "RESUME_FROM_P1 requires the P1 checkpoint and merged G0 data" >&2
    exit 2
  fi
  if [[ -e "$RUN_ROOT/p1_lock.json" || -e "$RUN_ROOT/g1" ]]; then
    echo "Refusing resume because P1 lock or G1 output already exists" >&2
    exit 2
  fi
  p1_lock_job=$(submit_job \
    --job-name="${CONFIG_NAME}_p1lock" \
    --export=ALL,RUN_ROOT="$RUN_ROOT",OUTPUT_STAGE=p1,RESIDUAL_MARGIN="$P1_RESIDUAL_MARGIN",ECONOMIC_MARGIN_EUR="$P1_ECONOMIC_MARGIN_EUR",REQUIRED_HEADS="$COLLECTION_REQUIRED_HEADS",PROTOCOL_PREFIX="iterative_q_${CONFIG_NAME}",POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES="$MAX_OVERRIDES" \
    hpc/submit_iterative_q_lock.sh)
  job_manifest=("p1_lock=$p1_lock_job")
  previous_train_job=$p1_lock_job
  manifest_path="$RUN_ROOT/recovery_job_ids.txt"
elif [[ "$RESUME_FROM_G0" == "1" ]]; then
  if [[ ! -s "$RUN_ROOT/g0/train_merged.npz" \
     || ! -s "$RUN_ROOT/g0/validation_merged.npz" ]]; then
    echo "RESUME_FROM_G0 requires merged G0 train and validation data" >&2
    exit 2
  fi
  if [[ -e "$RUN_ROOT/p1" || -e "$RUN_ROOT/g1" ]]; then
    echo "Refusing G0 resume because P1 or G1 output already exists" >&2
    exit 2
  fi
  p1_job=$(submit_job \
    --job-name="${CONFIG_NAME}_p1" \
    --export=ALL,RUN_ROOT="$RUN_ROOT",OUTPUT_STAGE=p1,DATA_STAGES=g0,ENCODER_LR=0.0001,HEAD_LR=0.0003,FOLLOW_ANCHOR=0.5,CREATE_LOCK=1,RESIDUAL_MARGIN="$P1_RESIDUAL_MARGIN",ECONOMIC_MARGIN_EUR="$P1_ECONOMIC_MARGIN_EUR",REQUIRED_HEADS="$COLLECTION_REQUIRED_HEADS",PROTOCOL_PREFIX="iterative_q_${CONFIG_NAME}",OBSERVATION_INPUT="$OBSERVATION_INPUT",EXCLUDE_STATE_FEATURES="$EXCLUDE_STATE_FEATURES",POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES="$MAX_OVERRIDES",MODEL_SEED="$MODEL_SEED",STAGE_SAMPLING_TEMPERATURE="$STAGE_SAMPLING_TEMPERATURE",NEAR_DUPLICATE_WEIGHTING="$NEAR_DUPLICATE_WEIGHTING",NEAR_DUPLICATE_COSINE_THRESHOLD="$NEAR_DUPLICATE_COSINE_THRESHOLD",NEAR_DUPLICATE_RMS_THRESHOLD="$NEAR_DUPLICATE_RMS_THRESHOLD",ROOT_ADVANTAGE_WEIGHTING="$ROOT_ADVANTAGE_WEIGHTING",ROOT_ADVANTAGE_THRESHOLD_EUR="$ROOT_ADVANTAGE_THRESHOLD_EUR",ROOT_NO_IMPROVEMENT_WEIGHT="$ROOT_NO_IMPROVEMENT_WEIGHT",ROOT_MODERATE_IMPROVEMENT_WEIGHT="$ROOT_MODERATE_IMPROVEMENT_WEIGHT",ROOT_STRONG_IMPROVEMENT_WEIGHT="$ROOT_STRONG_IMPROVEMENT_WEIGHT" \
    hpc/submit_iterative_q_train.sh)
  job_manifest=("p1=$p1_job")
  previous_train_job=$p1_job
  manifest_path="$RUN_ROOT/job_ids.txt"
else
  if [[ -e "$RUN_ROOT" ]]; then
  ...
  g0_job=$(submit_job \
    ...
    --export=ALL,RUN_ROOT="$RUN_ROOT",TRAIN_START="$G0_TRAIN_START",TRAIN_COUNT="$G0_TRAIN_COUNT",VALIDATION_START="$G0_VALIDATION_START",VALIDATION_COUNT="$G0_VALIDATION_COUNT",CHUNK_SIZE="$G0_CHUNK_SIZE",G0_ROOT_FRACTIONS="$G0_ROOT_FRACTIONS",G0_ROOTS_PER_SEED="$G0_ROOTS_PER_SEED",SCENARIO_PROTOCOL="$SCENARIO_PROTOCOL",HARD_SCENARIO_PROBABILITY="$HARD_SCENARIO_PROBABILITY",FORECAST_CONTEXT_HOURS="$FORECAST_CONTEXT_HOURS",FUTURE_SUMMARY_WINDOWS_H="$FUTURE_SUMMARY_WINDOWS_H" \
...
for ((index = 0; index < ITERATIONS; index++)); do
  ...
  train_job=$(submit_job \
    ...
    --export=ALL,RUN_ROOT="$RUN_ROOT",OUTPUT_STAGE="$output_stage",DATA_STAGES="$data_stages",INITIAL_CHECKPOINT="$RUN_ROOT/$previous_stage/iterative_action_q.pt",ENCODER_LR="$encoder_lr",HEAD_LR="$head_lr",FOLLOW_ANCHOR=0.0,CREATE_LOCK="$create_lock",RESIDUAL_MARGIN="$ITER_RESIDUAL_MARGIN",ECONOMIC_MARGIN_EUR="$ITER_ECONOMIC_MARGIN_EUR",REQUIRED_HEADS="$COLLECTION_REQUIRED_HEADS",PROTOCOL_PREFIX="iterative_q_${CONFIG_NAME}",OBSERVATION_INPUT="$OBSERVATION_INPUT",EXCLUDE_STATE_FEATURES="$EXCLUDE_STATE_FEATURES",POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES="$MAX_OVERRIDES",MODEL_SEED="$MODEL_SEED",STAGE_SAMPLING_TEMPERATURE="$STAGE_SAMPLING_TEMPERATURE",NEAR_DUPLICATE_WEIGHTING="$NEAR_DUPLICATE_WEIGHTING",NEAR_DUPLICATE_COSINE_THRESHOLD="$NEAR_DUPLICATE_COSINE_THRESHOLD",NEAR_DUPLICATE_RMS_THRESHOLD="$NEAR_DUPLICATE_RMS_THRESHOLD",ROOT_ADVANTAGE_WEIGHTING="$ROOT_ADVANTAGE_WEIGHTING",ROOT_ADVANTAGE_THRESHOLD_EUR="$ROOT_ADVANTAGE_THRESHOLD_EUR",ROOT_NO_IMPROVEMENT_WEIGHT="$ROOT_NO_IMPROVEMENT_WEIGHT",ROOT_MODERATE_IMPROVEMENT_WEIGHT="$ROOT_MODERATE_IMPROVEMENT_WEIGHT",ROOT_STRONG_IMPROVEMENT_WEIGHT="$ROOT_STRONG_IMPROVEMENT_WEIGHT" \
    hpc/submit_iterative_q_train.sh)
  job_manifest+=(
    "$stage=$rollout_job"
  ...
  if [[ "$EVAL_EACH_STAGE" == "1" && "$output_stage" != "$final_stage" ]]; then
    stage_eval_name="${CONFIG_NAME}_${output_stage}"
    stage_eval_job=$(submit_job \
      --dependency=afterok:"$train_job" \
      --job-name="${CONFIG_NAME}_${output_stage}e" \
      --export=ALL,RUN_ROOT="$RUN_ROOT",EVAL_NAME="$stage_eval_name",FINAL_STAGE="$output_stage",SCENARIO_PROTOCOL="$SCENARIO_PROTOCOL",HARD_SCENARIO_PROBABILITY="$HARD_SCENARIO_PROBABILITY",FORECAST_CONTEXT_HOURS="$FORECAST_CONTEXT_HOURS",FUTURE_SUMMARY_WINDOWS_H="$FUTURE_SUMMARY_WINDOWS_H",POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES="$MAX_OVERRIDES",EVAL_SEEDS="$EVAL_SEEDS",VALIDATION_ONLY="$VALIDATION_ONLY" \
      hpc/submit_iterative_q_eval.sh)
    job_manifest+=("${output_stage}_eval=$stage_eval_job")
  fi
...
eval_job=$(submit_job \
  ...
  --export=ALL,RUN_ROOT="$RUN_ROOT",EVAL_NAME="$CONFIG_NAME",FINAL_STAGE="$final_stage",SCENARIO_PROTOCOL="$SCENARIO_PROTOCOL",HARD_SCENARIO_PROBABILITY="$HARD_SCENARIO_PROBABILITY",FORECAST_CONTEXT_HOURS="$FORECAST_CONTEXT_HOURS",FUTURE_SUMMARY_WINDOWS_H="$FUTURE_SUMMARY_WINDOWS_H",POLICY_WINDOWS_H="$POLICY_WINDOWS_H",MAX_OVERRIDES="$MAX_OVERRIDES",EVAL_SEEDS="$EVAL_SEEDS",VALIDATION_ONLY="$VALIDATION_ONLY" \
  hpc/submit_iterative_q_eval.sh)
...
} | tee "$manifest_path"
{
  ...
  printf 'scenario_protocol=%s\n' "$SCENARIO_PROTOCOL"
  printf 'hard_scenario_probability=%s\n' \
    "$HARD_SCENARIO_PROBABILITY"
  printf 'forecast_context_hours=%s\n' "$FORECAST_CONTEXT_HOURS"
  printf 'future_summary_windows_h=%s\n' "$FUTURE_SUMMARY_WINDOWS_H"
  printf 'observation_input=%s\n' "$OBSERVATION_INPUT"
  printf 'exclude_state_features=%s\n' "$EXCLUDE_STATE_FEATURES"
  printf 'p1_residual_margin=%s\n' "$P1_RESIDUAL_MARGIN"
  printf 'p1_economic_margin_eur=%s\n' "$P1_ECONOMIC_MARGIN_EUR"
  printf 'iteration_residual_margin=%s\n' "$ITER_RESIDUAL_MARGIN"
  printf 'iteration_economic_margin_eur=%s\n' \
  ...
  printf 'stage_sampling_temperature=%s\n' \
    "$STAGE_SAMPLING_TEMPERATURE"
  printf 'near_duplicate_weighting=%s\n' "$NEAR_DUPLICATE_WEIGHTING"
  printf 'near_duplicate_cosine_threshold=%s\n' \
    "$NEAR_DUPLICATE_COSINE_THRESHOLD"
  printf 'near_duplicate_rms_threshold=%s\n' \
    "$NEAR_DUPLICATE_RMS_THRESHOLD"
  printf 'root_advantage_weighting=%s\n' "$ROOT_ADVANTAGE_WEIGHTING"
  printf 'root_advantage_threshold_eur=%s\n' \
