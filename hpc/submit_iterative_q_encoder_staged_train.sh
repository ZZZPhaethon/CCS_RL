#!/usr/bin/env bash
#SBATCH --job-name=ccs_iq_enc_stage
...
#SBATCH --array=0-2
#SBATCH -o logs/iterative_q_encoder_staged-%A_%a.out
#SBATCH -e logs/iterative_q_encoder_staged-%A_%a.err
...
train_stage() {
  ...
  python -u scripts/train_iterative_action_q.py \
    ...
    "${initial_args[@]}" \
    --out-dir "$OUT_ROOT/$ENCODER/$output_stage" \
...
train_stage p2 2 0.00005 0.00015 0.0 \
  "$OUT_ROOT/$ENCODER/p1/iterative_action_q.pt"
train_stage p3 3 0.00003 0.0001 0.0 \
  "$OUT_ROOT/$ENCODER/p2/iterative_action_q.pt"
train_stage p4 4 0.00003 0.0001 0.0 \
  "$OUT_ROOT/$ENCODER/p3/iterative_action_q.pt"
