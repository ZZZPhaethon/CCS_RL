...
RESULT_ROOT="$PROJECT_DIR/experiments_results/E1/ablation_rolling_milp_h72_r24_t600s_cplex222_seeds_9000031-9000060_${RUN_LABEL}"
LOCK_DIR="$PROJECT_DIR/experiments_results/rolling_milp_h72_r24_t600s_9000031_9000060_${RUN_LABEL}_lock"
LOCK_PATH="$LOCK_DIR/configuration_lock.txt"
JOB_MANIFEST="$LOCK_DIR/job_manifest.txt"
SUBMIT_SCRIPT="$PROJECT_DIR/hpc/submit_rolling_milp_h72_r24_t600s_array.sh"

if [[ "$CONCURRENCY" != "24" ]]; then
  echo "H72 concurrency is locked to 24 tasks." >&2
  exit 2
fi
...
if [[ -e "$RESULT_ROOT" || -e "$LOCK_DIR" ]]; then
  echo "Refusing repeated H72 run: output or lock already exists." >&2
  exit 3
fi
...
print("h72_r24_t600s_static_configuration_check=passed")
PY

mkdir "$LOCK_DIR"
{
  printf 'run_type=rolling_horizon_ablation\n'
  ...
  printf 'rolling_horizon_hours=72\n'
  ...
  printf 'interpretation=horizon_ablation_not_replacement_for_h168_formal_result\n'
  ...
  printf 'submit_script_sha256=%s\n' "$(sha256sum hpc/submit_rolling_milp_h72_r24_t600s_array.sh | awk '{print $1}')"
  printf 'launcher_sha256=%s\n' "$(sha256sum hpc/launch_rolling_milp_h72_r24_t600s.sh | awk '{print $1}')"
} > "$LOCK_PATH"

PURPOSE="horizon_ablation_E1_rolling_milp_h72_r24_cplex222_600s"
JOB_ID=$(sbatch --parsable \
  --job-name=rolling_h72_t600 \
  --array="0-29%$CONCURRENCY" \
  --mem=8G \
  -o "logs/rolling_h72_t600-%A_%a.out" \
  -e "logs/rolling_h72_t600-%A_%a.err" \
