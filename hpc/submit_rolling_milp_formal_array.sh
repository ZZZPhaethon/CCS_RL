#!/usr/bin/env bash
#SBATCH --job-name=rolling_milp_formal
...
#SBATCH -o logs/rolling_milp_formal-%A_%a.out
#SBATCH -e logs/rolling_milp_formal-%A_%a.err
...
RESULT_ROOT="${RESULT_ROOT:-$PROJECT_DIR/experiments_results/E1/rolling_milp_formal_9000031_9000060_cplex222_300s}"
PURPOSE="formal_test_E1_rolling_milp_cplex222_300s"
...
if (( ${SLURM_CPUS_PER_TASK:-0} != 4 )); then
  echo "Formal Rolling MILP requires exactly four allocated CPUs." >&2
  exit 2
fi
...
echo "time_limit_seconds_per_replan=300"
