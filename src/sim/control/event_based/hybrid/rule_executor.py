...
from sim.control.baselines import greedy_shuttle_policy, make_cluster_shuttle_policy
from sim.environment import CCSEnv, VESSEL_WAIT
...
def _environment_from_context(context: Mapping[str, Any]) -> CCSEnv:
    ...
    if not isinstance(env, CCSEnv):
        raise TypeError("context['env'] must be a reset sim.environment.CCSEnv.")
