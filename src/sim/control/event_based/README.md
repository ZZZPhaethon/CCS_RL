...
Synced from `origin/event-based` at
`9117ead8e7556be8406c9f683b4622abcdd789c1` and adapted to the local
`sim.control.event_based` namespace. All controllers in this package use the
same physical implementation under `sim`; no physical constraint is duplicated
inside this directory.

本目录是 CCS 项目的算法层，位于 `src/sim/` 之外。它负责“在什么目标下
运行”，而 `src/sim/` 仍是唯一的物理事实来源，负责“这个目标是否可以执行、
实际发生了什么、产生了什么成本和排放”。

This directory is the algorithm layer, intentionally outside `src/sim/`.
It decides *which operating goal to pursue*. `src/sim/` remains the single
...
sim.environment → simulator → physical constraints and reward
```
...
1. Compare the included rule and native-MPC executors through identical
   ...
   budget is available.
2. Add deterministic and rolling-MPC baselines under `src/sim/control/event_based/baselines/`;
   use identical scenarios, seeds, forecasts, and metrics.
3. Add an event-triggered or 12–24 h goal-level RL policy under
   `src/sim/control/event_based/rl/`. Its reward must be computed from realised simulator
...
2. 在 `src/sim/control/event_based/baselines/` 中加入确定性和滚动 MPC 基线，确保场景、随机种子、
   预报与指标完全一致。
3. 在 `src/sim/control/event_based/rl/` 中加入事件触发或每 12–24 小时决策一次的目标级 RL；奖励必须
   来自实际仿真结果，不能来自计划吨数。
...
The high-level PPO factory defaults to the local formal comparison protocol:
169 h forecast context, block weather, and warm-started reservoir pressure.
These settings are written to each run's `config.json`; older runs without
these keys retain their original 72 h/window/cold-start behaviour when loaded.
...
Those belong in `src/sim/entities`, `src/sim/operations`, and
`src/sim/network.py`, where all controllers receive the same treatment.

不要在此处加入物理能力、压力方程或流量裁剪规则；它们应位于
`src/sim/entities`、`src/sim/operations` 和 `src/sim/network.py`，以保证
所有控制器受到同一套物理约束。
